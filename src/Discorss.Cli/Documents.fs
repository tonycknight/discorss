...
module DocumentsConsole =
    ...
    let screenLayout () =
        ...
        let instructions = 
            Panel("Press Q to quit, O to open page, any key to continue." |> Console.yellow |> render)
    ...
    let updateDocumentsLayout (layout: Layout) (document: Document option) =
        ...
        let title (doc: Document option) =
            doc |> Option.map (fun d -> d.title |> Strings.escapeMarkup |> Console.cyan) |> Option.defaultValue "" |> render
                    ...
                    .StartAsync(fun ctx -> 
                        task {
                            ...
                            while not quit do
                                ...
                                try
                                    "Fetching..." |> Console.cyan |> Console.italic |> DocumentsConsole.updateStatus mainLayout
                                    ...
                                    r |> Option.map (fun _ -> "") 
                                      |> Option.defaultValue (("No article found." |> Console.red) + (" Hit Enter to try again." |> Console.cyan))
