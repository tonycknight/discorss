...
type FeedsActorState = { feedActors: Map<string, IActor> }
...
type FeedsActor(parent: IActor, config: AppConfiguration, http: IInternalHttpClient) as self =
    ...
    let getStats inbox state =
        async {
            ...
            return
                { myStats with
                    childStats = feedStats |> List.ofArray }
        }
    ...
    let queryFeeds () =
        async {
            ...
            return
                match r with
                | HttpRequestResponse.HttpOkRequestResponse(_, body) ->
                    body |> Newtonsoft.Json.JsonConvert.DeserializeObject<Feeds.FeedInfo[]> |> Some
                | _ -> None
        }
    ...
    let getOrCreateFeed (state: FeedsActorState) uri =
        let state, actor =
            ...
            | _ ->
                let actor = new FeedActor(self, config, http, uri) :> IActor

                let state =
                    { state with
                        feedActors = state.feedActors |> Map.add uri actor }
    ...
    let removeFeed (state: FeedsActorState) uri =
        { state with
            feedActors = state.feedActors |> Map.remove uri }

    let actor =
        MailboxProcessor<ActorMessage>.Start(fun inbox ->
            let rec loop (state: FeedsActorState) =
                async {
                    ...
                    let! state =
                        ...
                        | ActorMessage.QueryFeeds rc ->
                            async {
                                state.feedActors |> Seq.map (fun kv -> kv.Key) |> Array.ofSeq |> rc.Reply
                                return state
                            }
